...
{
	int a, b, result;
	Intro(); //Intro()함수 호출, ()안에 인수가 없음     함수 앞에 형이 없음, 반환되는 것이 없음
	a = Input(); //Input()함수 호출 ()안에 인수가 없음
	b = Input(); //Input()함수 호출 ()안에 인수가 없음
	result = Add(a, b); //Add 함수 호출, 인수 있음
	Result_Print(result); 
	return 0;
}
...
int Input(void) //사용자 정의 함수, 매개변수 없음
{
	...
	return input; //값을 반환  int input() --> int는 반환되는 값의 형 의미
}

int Add(int i, int j) //int i, int j -> 매개변수(형 선언)  a -> i       b -> j
{
